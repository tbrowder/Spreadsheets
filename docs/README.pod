...
Note the name of the module is plural. Not yet fully implemented (NYI) but planned, you can use a template
for transforming an existing worksheet into an XLSX file.
For example, using one or more of the input file formatscabove for all or partial
